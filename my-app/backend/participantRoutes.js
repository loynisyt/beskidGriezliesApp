...
// Delete a participant from a workout
...
router.get('/attendance', async (req, res) => {
    ...
    try {
        // Get total workouts count filtered by season if provided
        ...
        const totalResult = await pool.query(totalWorkoutsQuery, totalWorkoutsParams);
        ...
        // Get participation count per user filtered by season if provided
        ...
        const attendanceData = participationResult.rows.map(row => ({
            ...
            attendance_percentage: totalWorkouts > 0 ? ((row.participated_workouts / totalWorkouts) * 100).toFixed(2) : '0.00',
        }));
...
// Optional query params: season, month (YYYY-MM format)
router.get('/users/:userId/participation', async (req, res) => {
    const { userId } = req.params;
    const { season, month } = req.query;

    try {
        // Build base query for total workouts
        let totalWorkoutsQuery = 'SELECT COUNT(*) FROM workouts';
        let totalWorkoutsParams = [];
        let whereClauses = [];
        ...
        if (month) {
            whereClauses.push("to_char(date, 'YYYY-MM') = $" + (totalWorkoutsParams.length + 1));
        ...
        // Build query for participated workouts count
        ...
        if (month) {
            participatedQuery += season ? ' AND to_char(w.date, \'YYYY-MM\') = $3' : ' AND to_char(w.date, \'YYYY-MM\') = $2';
            participatedParams.push(month);
        }

        const totalResult = await pool.query(totalWorkoutsQuery, totalWorkoutsParams);
        const participatedResult = await pool.query(participatedQuery, participatedParams);
